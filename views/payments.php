    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        res.data.forEach(value => {
                            ...
                            if (value.approve_transaction.toLowerCase() == "no")
                                html += `<td><a statusID=${value.pay_id} data-toggle="tooltip" data-placement="top" title='Update Status On Payment ${value.pay_id}' class='btn btn-danger block'><i class="fa-solid fa-ban"></i></a></td>`
                            else
                                html += `<td><a statusID=${value.pay_id} class='btn btn-success active'>
                            <i class="fa-solid fa-check"></i></a></td>`
                            html += `<td>
                            <a delID=${value.pay_id} class='btn btn-danger delete'><i class="fa-solid fa-circle-minus"></i></a>
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            deletePayment(id, (res) => {
                                swal("Payment data has been removed!", {
                                    icon: "success",
                                });
                                readPaymentList();
            ...
            $(document).on("click", "a.active", function() {
                ...
                swal({
                        title: "Are you declining to authorize or proceed with this payment?",
                        // text: "Do You want to make this Payment not approving?",
                        icon: "warning",
                        buttons: ["cancel", "Yes"],
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            activate(id, "no", (res) => {
                                swal("Payemt Status  Has been updated successfully!", {
                                    icon: "success",
                                });
                                readPaymentList();
                            })
            ...
            $(document).on("click", "a.block", function() {
                ...
                swal({
                        title: "Are you authorizing or confirming this payment?",
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            activate(id, "yes", (res) => {
                                swal("Payment Status Has been updated successfully!", {
                                    icon: "success",
                                });
                                readPaymentList();
            ...
            // delete
            function deletePayment(id, response) {
                $.ajax({
                    method: "DELETE",
                    url: "http://localhost:4200/payments/" + id,
                    success: function(res) {
                        response(res)
                    },
                    error: function(res) {
            ...
            function activate(id, status, response) {
                ...
                $.ajax({
                    ...
                    url: "http://localhost:4200/payments/updateStatus",
                    success: function(res) {
                        response(res);
                    },
                    error: function(res) {
